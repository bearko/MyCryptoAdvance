...
縦に3分割している。スティックの居場所を固定して、存在と判定範囲がひと目で分かるようにした。

| 位置 | 高さ | 内容 |
|---|---|---|
| 上 | 50% | ゲーム画面。右上に表示倍率（×2〜×5）の切り替え |
| 中 | 20% | ミニマップ。現在地・表示中の範囲・宝箱、左右にコインと宝箱の数 |
| 下 | 30% | スティック画面。常時スティックを表示し、**このペイン全体が受付範囲** |

スティックは触った位置へ移動して効き、離すと定位置へ戻る。

## いま出来ること

- MCHウォーリアーが8方向にアニメーションしながら歩く（待機 / 歩行 / 攻撃）
- スティックの倒し具合で歩く速度が変わる
- 表示倍率を ×2〜×5 で変更でき、選んだ倍率は localStorage に残る
...
│   ├── game.js           メインループ / カメラ / 倍率 / ミニマップ / 宝箱
│   ├── input.js          仮想スティック（下30%ペイン）＋キーボード
│   ├── worldmap.js       フィールド生成・当たり判定・描画・ミニマップ
